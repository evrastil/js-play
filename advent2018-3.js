const puzzle3 = (input) => {
    ...
    const res = Object.entries(arr.reduce((boxes, id) => {
        let index = 0
        const letters = {}
        while (id.length > index) {
            letters[id.charAt(index)] = letters[id.charAt(index)] ? letters[id.charAt(index)] + 1 : 1
            index++
        }
        [...new Set(Object.entries(letters).map(([att, count]) => count))].forEach(c => {
            if (c > 1 && c < 4) {
                boxes[c] = boxes[c] ? boxes[c] + 1 : 1
            }
        })
        return boxes
    }, {})).reduce((agg, [key, val]) => {
...
puzzle3('bpacnmelhhzpygfsjoxtvkwuor\n' +
